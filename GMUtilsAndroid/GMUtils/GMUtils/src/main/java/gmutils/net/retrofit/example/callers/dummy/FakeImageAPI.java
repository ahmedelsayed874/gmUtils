...
public class FakeImageAPI implements ImageAPIs {
    ...
    public void post(String text, Bitmap image, OnResponseReady2o<Object, ImageResponse> callback) {
        FakeData.run0((s) -> {
            ...
            if (s == null) {
                r.setCallbackStatus(BaseResponse.Status.ConnectionFailed);
                r._code = 0;
            } else {
                if (s) {
                    r.setData(new Object());
                    r.setCallbackStatus(BaseResponse.Status.Succeeded);
                    r._code = 200;
                } else {
                    r.setCallbackStatus(BaseResponse.Status.Error);
                    r._code = 400;
                }
            }
