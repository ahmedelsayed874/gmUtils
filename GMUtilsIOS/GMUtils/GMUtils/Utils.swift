...
class Utils {
    ...
    static func appName() -> String {
        let info = Bundle.main.infoDictionary?["CFBundleName"]
        let appName = info as? String
        return appName ?? ""
    }
    
    static func appIdentifier() -> String {
        let info = Bundle.main.infoDictionary?["CFBundleIdentifier"]
        let identifier = info as? String
        return identifier ?? ""
    }
    ...
    static func appUrlOnAppStore(for id: String) -> String {
    ...
    static func calcPixelsInCM(_ cm: Double) -> Int {
        //An iOS point is equivalent to 1/163 of an inch.
        //let onePointPexils: Double = Double(UIScreen.main.scale) //1 (non-retina)
        let oneInchPoints = 163.0 //pt
        let oneCMPoints = oneInchPoints / 2.54 //pt
        let pointCount = oneCMPoints * cm
        let pixels = pointCount //* onePointPexils
        return Int(pixels)
    }
